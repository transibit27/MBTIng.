...
<body>
                        ...
                        for(var i in data){
                            ...
                            if(data[i].userEmail == "${sessionScope.loginMember.email}"){
                                ...
                                if(loginList.indexOf(data[i].masterEmail) != -1){
                                    $div = $("<div class='chatList_box enterRoomList' onclick='enterRoom(this);'>").attr("id",data[i].roomNo).attr("email",data[i].masterEmail);
                                }
                                // 현재 판매자가 로그아웃 상태 일 때
                                else{
                                    $div = $("<div class='chatList_box2 enterRoomList' onclick='enterRoom(this);'>").attr("id",data[i].roomNo).attr("email",data[i].masterEmail);
                                }
                            ...
                            // 자신이 판매자 입장일 때
                                ...
                                if(loginList.indexOf(data[i].userEmail) != -1){
                                    $div = $("<div class='chatList_box enterRoomList' onclick='enterRoom(this);'>").attr("id",data[i].roomNo).attr("email",data[i].userEmail);
                                }
                                // 현재 구매자가 로그아웃 상태 일 때
                                else{
                                    $div = $("<div class='chatList_box2 enterRoomList' onclick='enterRoom(this);'>").attr("id",data[i].roomNo).attr("email",data[i].userEmail);
                                }                                
    ...
    <script>
        ...
        // 웹소켓
         ...
         function connect() {
             // 웹소켓 주소"ws://localhost:8007/websocket/member.do";

             var wsUri = "ws://localhost:8081/mbting/chat.do";
             // 소켓 객체 생성
         ...
         function onOpen() {
             ...
             const data = {
                    "roomNo" : roomNo,
                    "name" : "${ sessionScope.loginMember.name }",
        ...
        function sendMessage(message){
         ...
         function onMessage(evt) {
            ...
            const data = {
             ...
             if(data.email != "${ sessionScope.loginMember.email }"){
                CheckLR(data);
             }
        }
